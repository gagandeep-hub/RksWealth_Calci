/**
 * Home Planning Formula
 * 
 * @param {number} currentCost - Current estimated cost of the house
 * @param {number} years - Years until planning to buy the house
 * @param {number} returnRate - Expected return rate on investments in %
 * @param {number} inflationRate - Expected inflation rate for property in %
 * @returns {object} { futureCost, lumpsumRequired, sipRequired }
 */
export const calculateHomePlanning = (currentCost, years, returnRate, inflationRate) => {
...
/**
 * Generates year-wise breakdown for Home Planning
 */
export const getHomePlanningBreakdown = (currentCost, years, returnRate, inflationRate) => {
  if (years <= 0 || currentCost <= 0) return [];

  const breakdown = [];
  const { sipRequired } = calculateHomePlanning(currentCost, years, returnRate, inflationRate);
