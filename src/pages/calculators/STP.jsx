...
import { ArrowLeftRight, IndianRupee, Percent, Calendar, Download, Share2, Info } from 'lucide-react';
...
import { calculateSTP, getStpBreakdown } from '../../utils/stpFormula';

const STP = () => {
  const [sourceInvestment, setSourceInvestment] = useState(100000);
  const [transferAmount, setTransferAmount] = useState(10000);
  const [years, setYears] = useState(5);
  const [sourceRate, setSourceRate] = useState(5);
  const [destRate, setDestRate] = useState(12);

  const [results, setResults] = useState({ totalValue: 0, totalInvested: 0, totalReturns: 0, finalSourceBalance: 0, finalDestBalance: 0 });
  const [breakdown, setBreakdown] = useState([]);

  useEffect(() => {
    const res = calculateSTP(sourceInvestment, transferAmount, sourceRate, destRate, years);
    setResults(res);
    setBreakdown(getStpBreakdown(sourceInvestment, transferAmount, sourceRate, destRate, years));
  }, [sourceInvestment, transferAmount, sourceRate, destRate, years]);
      ...
      <div className="mb-10">
        <nav className="flex mb-4 text-sm font-medium text-gray-500 dark:text-gray-400">
          <ol className="flex items-center space-x-2">
            ...
            <li><span>/</span></li>
            <li className="text-blue-600 dark:text-blue-400">STP Calculator</li>
          </ol>
        </nav>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">
          Calculate returns when you transfer a fixed amount regularly from one mutual fund to another.
        </p>
      </div>
          ...
          <div className="mt-12 p-6 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
            <div className="space-y-4">
              <div className="flex flex-col gap-1">
                <span className="text-gray-900 dark:text-gray-100 font-medium">Invested Amount</span>
                <span className="text-gray-900 dark:text-white font-black text-xl">{formatCurrency(results.totalInvested)}</span>
              </div>
              <div className="h-px bg-gray-100 dark:bg-gray-700" />
              <div className="flex flex-col gap-1">
                <span className="text-gray-900 dark:text-gray-100 font-medium">Balance in Source Fund</span>
                <span className="text-gray-900 dark:text-white font-black text-xl">{formatCurrency(results.finalSourceBalance)}</span>
              </div>
              <div className="h-px bg-gray-100 dark:bg-gray-700" />
              <div className="flex flex-col gap-1">
                <span className="text-gray-900 dark:text-gray-100 font-medium">Amount Transferred to Destination Fund</span>
                <span className="text-gray-900 dark:text-white font-black text-xl">{formatCurrency(results.totalTransferred || 0)}</span>
              </div>
              <div className="h-px bg-gray-100 dark:bg-gray-700" />
              <div className="flex flex-col gap-1">
                <span className="text-gray-900 dark:text-gray-100 font-medium">Expected Amount</span>
                <span className="text-gray-900 dark:text-white font-black text-xl">{formatCurrency(results.totalValue)}</span>
              </div>
            </div>
          </div>
        </motion.div>
            ...
            <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col justify-center">
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Expected Amount</p>
                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{formatCurrency(results.totalValue)}</p>
                  </div>
                </div>
