...
import { Heart, IndianRupee, Percent, Calendar, Download, Share2, Info, Users } from 'lucide-react';
...
import { calculateMarriage, getMarriageBreakdown } from '../../utils/marriageFormula';
...
const Marriage = () => {
  ...
  useEffect(() => {
    // Ensure marriage age is greater than current age
    const validMarriageAge = Math.max(currentAge + 1, marriageAge);
    if (validMarriageAge !== marriageAge) {
      setMarriageAge(validMarriageAge);
    } else {
      const res = calculateMarriage(currentAge, marriageAge, currentExpense, returnRate, inflationRate);
      setResults(res);
      setBreakdown(getMarriageBreakdown(currentAge, marriageAge, currentExpense, returnRate, inflationRate));
    }
  }, [currentAge, marriageAge, currentExpense, returnRate, inflationRate]);
    ...
    <div className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="mb-10">
        <nav className="flex mb-4 text-sm font-medium text-gray-500 dark:text-gray-400">
          <ol className="flex items-center space-x-2">
            ...
            <li><span>/</span></li>
            <li className="text-emerald-600 dark:text-emerald-400">Marriage Planning</li>
          </ol>
        </nav>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">
          Plan for your dream wedding by estimating future costs and required investments.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        >
          ...
          <div className="mt-12 p-6 bg-pink-50 dark:bg-pink-500/10 rounded-2xl border border-pink-100 dark:border-pink-500/20">
            <h3 className="text-pink-800 dark:text-pink-400 font-bold mb-4 flex items-center gap-2">
              <Info className="w-4 h-4" />
              Quick Summary
            </h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">Current Marriage Expenses</span>
                <span className="text-gray-900 dark:text-gray-100 font-bold">{formatCurrency(currentExpense)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">Future Marriage Expenses</span>
                <span className="text-gray-900 dark:text-gray-100 font-bold">{formatCurrency(results.futureExpense)}</span>
              </div>
              <div className="h-px bg-pink-200 dark:bg-pink-500/20" />
              <div className="flex justify-between items-center">
                <span className="text-gray-800 dark:text-gray-200 font-bold">Planning Through Lumpsum</span>
                <span className="text-pink-600 dark:text-pink-400 font-bold">{formatCurrency(results.lumpsumRequired)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-800 dark:text-gray-200 font-bold">Planning Through SIP</span>
                <span className="text-pink-600 dark:text-pink-400 text-xl font-black">{formatCurrency(results.sipRequired)} /mo</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col items-center">
              <h3 className="text-gray-700 dark:text-gray-200 font-bold mb-6 w-full">Corpus Breakup (SIP)</h3>
              <PieChart invested={results.sipRequired * (marriageAge - currentAge) * 12} returns={results.futureExpense - (results.sipRequired * (marriageAge - currentAge) * 12)} />
            </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{formatCurrency(results.sipRequired * (marriageAge - currentAge) * 12)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-gray-800 dark:text-gray-100">{formatCurrency(results.futureExpense - (results.sipRequired * (marriageAge - currentAge) * 12))}</p>
                  </div>
                </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex gap-2">
                <button className="p-2 bg-gray-100 dark:bg-gray-700 rounded-lg hover:text-pink-500 transition-colors"><Share2 className="w-5 h-5" /></button>
              </div>
            </div>
